...
public class A5_Das_Ende_der_Sitzung_eintragen {
    public A5_Das_Ende_der_Sitzung_eintragen() {
        ...
        Drucken.getInstance().print_Titel("Aufgabe 5");

        Sitzungen.getInstance().Anzeigen(Sitzungen.getInstance().getAktiveSitzung().getID());

        if (Tools.getInstance().frage_Ja_Nein("Ende der Sitzung eintragen")) {
            Timestamp ende = Tools.getInstance().getTimestamp("Ende der Sitzung");
